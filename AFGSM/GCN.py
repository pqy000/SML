...
class GCN:
    ...
    def train(self, split_train, split_val, Z_obs, patience=30, n_iters=200, print_info=True):
        ...
        for it in range(n_iters):
            ...
            f1_micro, f1_macro, acc = eval_class(split_val, self, np.argmax(Z_obs, 1))
            print('it: %4d, train loss: %.4f, val acc: %.4f' % (it, _loss, acc))
...
def eval_class(ids_to_eval, self, z_obs):
    ...
    acc = np.mean(test_pred == test_real)
    return f1_score(test_real, test_pred, average='micro'), f1_score(test_real, test_pred, average='macro'), acc
...
class GCN_Batch:
    ...
    def train(self, _An, AX, split_train, split_val, Z, patience=30, n_iters=200, batchsize=1000, print_info=True):
        ...
        if self.retrain:
            self.load_model()
        for it in range(n_iters):
            for An_batch, AX_batch, Y_batch in iterate_minibatches_listinputs(_An, AX, Z, split_train, min(493486, len(split_train))):
            ...
            for An_val, AX_val, Y_val in iterate_minibatches_listinputs(_An, AX, Z, split_val,
                ...
                break
                '''
            print('it: %4d, train loss: %.4f, val loss: %.4f, val acc: %.4f' % (it, loss, val_loss, val_acc))
            if (val_acc > best_acc):
                p_count = 0
...
def iterate_minibatches_listinputs(An, AX, labels, train_index, batchsize, random_shuffle=True):
    ...
    for start_id in range(0, numSamples, batchsize):
        excerpt = np.asarray(train_index)[indices[start_id:min(start_id + batchsize, numSamples)]]
        An_expt = An[excerpt]
        onehup = np.unique(np.nonzero(An_expt)[1])
